...
class Deformer(nn.Module):
    def cnn_block(self, out_chan, kernel_size, num_chan):
        return nn.Sequential(
            Conv2dWithConstraint(1, out_chan, kernel_size, padding=self.get_padding(kernel_size[-1]), max_norm=2),#out_chanx128xtemporal_dim(same as input temporal dim 440
            Conv2dWithConstraint(out_chan, out_chan, (num_chan, 1), padding=0, max_norm=2),#out_chanx1xtemporal_dim(440)
            nn.BatchNorm2d(out_chan),
            nn.ELU(),
            nn.MaxPool2d((1, 2), stride=(1, 2))#out_chanx1x0.5*temporal_dim
        )
    ...
    def __init__(self, *, num_chan, num_time, temporal_kernel, num_kernel=64,
        ...
        self.PEG = nn.Conv1d(in_channels=num_kernel, out_channels=num_kernel, kernel_size=temporal_kernel, padding=temporal_kernel//2)
